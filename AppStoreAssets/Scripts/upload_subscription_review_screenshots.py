...
FETCH_UPLOADS = ROOT / "AppStoreAssets" / "Scripts" / "fetch_subscription_review_screenshot_uploads.js"
...
def main():
    ...
    if not uploads:
        print("No subscription review screenshot reservations found", file=sys.stderr)
        return 1
    ...
    for item in uploads:
        ...
        if state not in {"AWAITING_UPLOAD", "UPLOAD_COMPLETE"}:
            print(f"Skipping {item['label']} in state {state}", file=sys.stderr)
            continue

        operations = item.get("uploadOperations") or []
        if not operations:
            print(f"Skipping {item['label']} because there are no upload operations", file=sys.stderr)
            continue
