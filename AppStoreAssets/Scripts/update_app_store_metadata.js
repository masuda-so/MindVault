...
9. If the test device supports Apple Foundation Models, open the AI organization or AI chat search flow. If unsupported, confirm that the app shows an explanatory unavailable state and does not offer an external AI fallback.`;
  ...
  function request(method, url, body) {
    ...
    return { method, url, status: xhr.status, response: parsed };
  }

  const results = [];

  results.push(request("PATCH", `https://appstoreconnect.apple.com/iris/v1/appStoreVersionLocalizations/${LOCALIZATION_ID}`, {
    data: {
      type: "appStoreVersionLocalizations",
      id: LOCALIZATION_ID,
      attributes: {
        description,
        keywords: "メモ,ノート,Markdown,知識グラフ,AI整理,オンデバイスAI,ローカル保存,リンク,タグ,検索,アイデア,日記,学習,仕事",
        marketingUrl,
        promotionalText: "Markdownメモを起動直後の知識グラフで見渡せます。リンク、タグ、オンデバイスAI提案を使い、考えのつながりをローカル優先で育てるノートアプリです。",
        supportUrl,
        whatsNew: "MindVault 1.0 の初回リリースです。Markdownメモ、知識グラフ、ローカル検索、オンデバイスAI整理の導線を追加しました。"
      }
    }
  }));

  results.push(request("PATCH", `https://appstoreconnect.apple.com/iris/v1/appInfoLocalizations/${APP_INFO_LOCALIZATION_ID}`, {
    data: {
  ...
  results.push(request("PATCH", `https://appstoreconnect.apple.com/iris/v1/appStoreVersions/${VERSION_ID}`, {
    data: {
  ...
  results.push(request("PATCH", `https://appstoreconnect.apple.com/iris/v1/appInfos/${APP_INFO_ID}/relationships/primaryCategory`, {
    data: { type: "appCategories", id: "PRODUCTIVITY" }
  }));

  results.push(request("PATCH", `https://appstoreconnect.apple.com/iris/v1/appInfos/${APP_INFO_ID}/relationships/secondaryCategory`, {
    data: { type: "appCategories", id: "UTILITIES" }
  }));

  results.push(request("PATCH", `https://appstoreconnect.apple.com/iris/v1/ageRatingDeclarations/${AGE_RATING_ID}`, {
    data: {
  ...
  results.push(request("POST", "https://appstoreconnect.apple.com/iris/v1/appStoreReviewDetails", {
    data: {
